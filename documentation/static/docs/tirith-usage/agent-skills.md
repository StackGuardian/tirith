...
Source: https://stackguardian.github.io/tirith/docs/tirith-usage/agent-skills/
Summary: Install the Tirith skill pack so a coding agent writes policies from the real vocabulary instead of inventing condition types that look plausible.
...
`tirith-migrate`, for translating existing Sentinel policies. No config file, and they are picked
...
The third skill, `tirith-migrate`, is for teams with existing HashiCorp Sentinel policies. It is a
