...
def _policy_label(policy_result: Dict) -> str:
    """`<name> — <policy name>`, falling back to the file name when meta says nothing."""
    name = policy_result.get("policy", "")
    meta = policy_result.get("meta") or {}
    title = meta.get("name") or meta.get("id") or ""
    return f"{name} — {title}" if title else name


def pretty_print_policy_set_result(set_result_dict: Dict, verbose: bool = False) -> None:
    """
    Print the aggregate result of running many policies against one input.

    Printing every policy in full does not scale: a pack has hundreds, and almost all of them
    skip, because a check only applies to plans that touch the resource it names. So failures
    are printed in full -- they are the reason anyone ran this -- and everything else is counted.
    `verbose` restores the full per-policy detail.

    :param set_result_dict: Result dictionary generated by `start_policy_set_evaluation`
    :param verbose:         Print every policy in full, not just the failures
    """
    policies = set_result_dict.get("policies", [])
    summary = set_result_dict.get("summary", {})

    errored = [p for p in policies if "final_result" not in p]

    for policy_result in policies:
        if not verbose and policy_result.get("final_result") is not False:
            continue
        print(TermStyle.str_with_style(_policy_label(policy_result), TermStyle.BOLD))
        if "final_result" in policy_result:
            pretty_print_result_dict(policy_result)
        else:
            for error in policy_result.get("errors", []):
                print(TermStyle.fail(f"- {error}"))
        print()

    if errored and not verbose:
        print(TermStyle.fail(f"Could not evaluate {len(errored)} policy/policies:"))
        for policy_result in errored:
            for error in policy_result.get("errors", []):
                print(TermStyle.fail(f"- {policy_result.get('policy', '')}: {error}"))
        print()

    print(
        f"{summary.get('total', 0)} policies · "
        f"{TermStyle.green(str(summary.get('passed', 0)) + ' passed')} · "
        f"{TermStyle.red(str(summary.get('failed', 0)) + ' failed')} · "
        f"{TermStyle.grey(str(summary.get('skipped', 0)) + ' skipped')}"
        + (f" · {TermStyle.fail(str(summary.get('errored', 0)) + ' errored')}" if summary.get("errored") else "")
    )
    # Said explicitly because it is the one number that surprises people: a skipped policy found
    # no resource of the type it checks, which is the normal outcome for most of a large pack.
    if summary.get("skipped"):
        print(TermStyle.grey("Skipped policies found no resource of the type they check."))
    print()

    final_result = set_result_dict.get("final_result")
    if final_result is True:
        print(TermStyle.success("✔ No policy failed"))
    elif final_result is False:
        print(TermStyle.fail(f"✘ {summary.get('failed', 0)} policy/policies failed"))
    else:
        print(TermStyle.skipped("= No policy reached a verdict"))
