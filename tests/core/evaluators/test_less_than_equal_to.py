...
def test_evaluate_passing():
    ...
    assert result == {
        "passed": True,
        "message": f"{json_format_value(evaluator_input1)} is less than equal to {json_format_value(evaluator_data1)}",
    }
...
def test_evaluate_failing():
    ...
    assert result == {
        "passed": False,
        "message": f"{json_format_value(evaluator_input2)} is not less than or equal to {json_format_value(evaluator_data2)}",
    }
