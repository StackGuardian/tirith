...
@mark.passing
def test_a_provider_error_is_not_surfaced_on_this_release_line():
    """
    Pins what the 1.0.x core does with a provider error that carries no `ProviderError`
    severity: the provider's own message is discarded and `None` is evaluated against the
    condition, so the comparison itself is what fails.

    The context still earns its place -- it names the evaluator the `NoneType` came from, which
    is otherwise unguessable from a bare TypeError. Surfacing the provider's message instead is
    a verdict change (1.1.0 does it) and does not belong in a patch release.
    """
    result = evaluate(
        {"operation_type": "total_monthly_cost", "resource_type": ["*"]},
        {"type": "LessThan", "value": 20},
    ...
    assert item["message"] == (
        "[all resources] total_monthly_cost: '<' not supported between instances of 'NoneType' and 'int'"
    )
